...
- **Cause**: The bridge cannot find the `codex` or `claude` binary in your `$PATH`.
- **Fix**:
  - Export the specific path: `export CODEX_BINARY=/path/to/codex`.
  - For the older wrapper only, set `CODEX_ADAPTER_MODE=legacy-agent` and point `CODEX_BINARY` at `codex-agent`.
