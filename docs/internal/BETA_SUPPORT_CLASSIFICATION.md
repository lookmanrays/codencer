...
| ChatGPT-style planner path via relay/cloud | `partial` | `compatibility` | `operator-packaged` | Post-beta flagship docs, smoke, bearer-token proof, and OAuth protected-resource metadata/challenge support make the Codencer-side remote MCP loop concrete for operator use. No universal ChatGPT product support is claimed. |
...
| Claude Desktop / `claude.ai` remote connector path via relay/cloud | `compatibility` | `compatibility` | `compatibility-only` | Codencer-side OAuth protected-resource metadata is proven, but product-side remote connector auth/setup remains outside repo proof and separate from the local `claude` execution adapter proof. |
