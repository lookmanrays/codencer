...
| --- | --- | --- | --- |
| `chatgpt-relay.mcp.json` | value-reference only | OAuth dev or front door | Copy endpoint/auth values into the current ChatGPT custom MCP connector setup flow. Static bearer import is not the product baseline. |
| `chatgpt-cloud.mcp.json` | value-reference only | OAuth front door | Same for cloud `/api/cloud/v1/mcp`; the Sprint 7 OAuth dev issuer is relay-local. |
