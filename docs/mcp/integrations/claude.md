# Claude-Style Planner — Codencer Walkthrough
...
For inherited beta support boundaries, start with [Public Beta Test Tracks](../../BETA_TESTING.md) and [Planner / Client Integration Notes](../integrations.md).
...
- [MCP connector in the Messages API](https://docs.anthropic.com/en/docs/agents-and-tools/mcp-connector)
...
Do not publish or rely on a remote-connector-via-`claude_desktop_config.json` claim. That would be false as of 2026-04-27.
...
Codencer's proven execution auth model on these planner-side surfaces is bearer-token based.
...
Codencer now also exposes OAuth protected-resource metadata and 401 bearer challenges:

- relay metadata: `https://<host>/.well-known/oauth-protected-resource/mcp`
- cloud metadata: `https://<host>/.well-known/oauth-protected-resource/api/cloud/v1/mcp`

Anthropic's current public docs for remote MCP include OAuth bearer-token flows and remote HTTP MCP. Treat the product-specific connector setup as a real compatibility boundary:

- Codencer's remote MCP surface is repo-documented and repo-tested.
- Anthropic's product-side remote connector UI/API flow is current external platform behavior.
- Bearer-header Claude Code-style setup is operator-packaged.
- OAuth protected-resource discovery is repo-proven, but the OAuth issuer/front door is operator-owned.
- Claude Desktop and `claude.ai` remote connector product setup remains `compatibility-only` because this repo does not directly exercise it.

If your Anthropic-side connector setup requires a full OAuth authorization flow, use an operator-owned auth front door or issuer before the Codencer MCP endpoint. Codencer itself does not issue OAuth authorization-code tokens.
...
- Bearer-token mode is repo-proven; OAuth protected-resource metadata/challenge is repo-proven; full Anthropic product OAuth setup is not repo-proven.
...
- Connector is added but tool calls fail with auth errors: verify bearer scopes for Claude Code-style clients, or verify your OAuth issuer/front door for product remote connector clients.
