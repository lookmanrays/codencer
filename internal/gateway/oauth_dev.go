...
func newOAuthDevService(cfg *Config) *oauthDevService {
	if cfg == nil || !cfg.OAuthDev.Enabled {
		return nil
	}
...
func (s *Server) handleOAuthAuthorizationServer(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, s.oauth.Metadata(s.baseURL(r)))
}
...
func (s *Server) handleOpenIDConfiguration(w http.ResponseWriter, r *http.Request) {
	...
	metadata := s.oauth.Metadata(s.baseURL(r))
...
func (s *Server) handleOAuthAuthorize(w http.ResponseWriter, r *http.Request) {
	...
	if operatorCode == "" {
		...
		_, _ = w.Write([]byte(s.oauth.AuthorizeForm(values)))
		return
	}
...
func (s *Server) handleOAuthToken(w http.ResponseWriter, r *http.Request) {
	...
	response, apiErr := s.oauth.ExchangeCode(r.Form, r.Header.Get("Authorization"), s.baseURL(r))
	if apiErr != nil {
		writeAPIError(w, apiErr.Status, apiErr.Code, apiErr.Message)
		return
	}
...
func (o *oauthDevService) AuthorizeForm(values url.Values) string {
	...
	return `<!doctype html><html><head><meta charset="utf-8"><title>Codencer Gateway OAuth Dev Approval</title></head><body>` +
		`<h1>Codencer Gateway OAuth Dev Approval</h1>` +
		`<p>Enter the operator approval code generated by <code>codencer setup self-host --enable-oauth-dev</code>.</p>` +
...
func (o *oauthDevService) CreateAuthorizationCode(values url.Values, baseURL string) (string, string, string, *apiError) {
	...
	if !o.resourceAllowed(resource, baseURL) {
		return "", "", "", &apiError{Status: http.StatusBadRequest, Code: "invalid_resource", Message: "OAuth token resource must target the Gateway MCP endpoint"}
	}
	scope := normalizeRequestedScopes(values.Get("scope"), o.cfg.Scopes)
	code, err := randomOpaqueToken(32)
	if err != nil {
		return "", "", "", &apiError{Status: http.StatusInternalServerError, Code: "gateway_internal_error", Message: err.Error()}
	}
	o.mu.Lock()
	defer o.mu.Unlock()
	o.pruneLocked(o.now())
...
func (o *oauthDevService) ExchangeCode(values url.Values, authorizationHeader, baseURL string) (map[string]any, *apiError) {
	...
	if !o.resourceAllowed(authCode.Resource, baseURL) {
		return nil, &apiError{Status: http.StatusBadRequest, Code: "invalid_resource", Message: "OAuth token resource must target the Gateway MCP endpoint"}
	}
	accessToken, err := randomOpaqueToken(32)
	if err != nil {
		return nil, &apiError{Status: http.StatusInternalServerError, Code: "gateway_internal_error", Message: err.Error()}
	}
...
func (o *oauthDevService) Authenticate(token string) (*authPrincipal, *apiError) {
	if o == nil {
