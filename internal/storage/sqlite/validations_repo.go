...
func (r *ValidationsRepo) ListByStep(ctx context.Context, stepID string) (map[string][]*domain.ValidationResult, error) {
	q := `
		SELECT v.attempt_id, v.name, v.command, v.state, v.passed, v.exit_code, v.stdout_ref, v.stderr_ref, v.error_msg, v.duration_ms, v.updated_at
		FROM validations v
...
func (r *ValidationsRepo) ListByAttempt(ctx context.Context, attemptID string) ([]*domain.ValidationResult, error) {
	q := `
		SELECT name, command, state, passed, exit_code, stdout_ref, stderr_ref, error_msg, duration_ms, updated_at
