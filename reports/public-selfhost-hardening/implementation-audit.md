...
| Default output does not leak local paths | Partially implemented | Default human output for init, config show, config profile/set commands, project init/status/scan, executor list/scan/test/default, setup self-host/relay, activation self-host, sync preview, submit, run list/get/status/events/cancel, run report, and run resume blocker output is redacted and tested; source/artifact Gateway API outputs are now checked for local path, daemon URL, token, and unsafe field leaks. Explicit `--json` and path/debug commands still include local `repo_root`, `daemon_url`, and `report_path` for operator tooling. |
