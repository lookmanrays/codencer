...
- Confirmed the real Codex path invokes `/Applications/Codex.app/Contents/Resources/codex` with `ALL_ADAPTERS_SIMULATION_MODE=0` and `CODEX_SIMULATION_MODE=0`.
...
- Real Codex artifact-backed RC subgate passed:
  ...
  - Evidence: daemon log shows `Adapter Execution: Starting process`, `adapter=codex`, `binary=/Applications/Codex.app/Contents/Resources/codex`.
...
- `CODENCER_E2E_REAL_EXECUTOR=codex CODENCER_E2E_REAL_EXECUTOR_COMMAND=/Applications/Codex.app/Contents/Resources/codex make verify-public-selfhost-rc` - failed by design with `NO-GO` after Codex passed and Claude/Antigravity proofs were missing
...
- Explicit local-to-Gateway sync/publish behavior remains unimplemented.
- Run history/audit pagination, filters, grouping, and local/synced/Gateway-submitted scopes remain incomplete against the full public self-host hardening spec.
