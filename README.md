...
| --- | --- | --- | --- | --- |
| Local production foundation | [docs/quickstart-local.md](docs/quickstart-local.md), [docs/local-production.md](docs/local-production.md), [docs/project-config.md](docs/project-config.md), [docs/runtime-supervisor.md](docs/runtime-supervisor.md), [docs/live-execution-matrix.md](docs/live-execution-matrix.md) | `make build` | `make verify-local-prod` and `make acceptance-local-production` | Unified `codencer` CLI, setup UX, project-local config, user-level registry, daemon execution, manifest runner, Relay/MCP proof, supervisor/watchdog/recovery, live matrix/readiness, acceptance reports, proof bundles, and release snapshots. |
